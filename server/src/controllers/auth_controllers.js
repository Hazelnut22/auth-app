...
export const register = catch_async(async (req, res) => {
  ...
  if (existing) return sendError(res, "Registration failed. Please try again.", 400);
  ...
  // 5. First password history entry
  await PasswordHistory.create({
    userId:       newUser._id,
    passwordHash: hashedPassword,
    createdAt:    new Date(),
...
export const login = catch_async(async (req, res) => {
  ...
  // 5. Timing-safe password check
  //    Always runs argon2.verify even if user not found — prevents timing attacks
  ...
  // 7. Success — reset counters + record last login
  await User.findByIdAndUpdate(user._id, {
    failedAttempts: 0,
    lockedUntil:    null,
  ...
  // 8a. MFA required — issue temp token, NOT a full session
  if (user.isMfaActive) {
...
export const status = catch_async(async (req, res) => {
  ...
  // Last 5 events from AuditLog model
...
// ── VERIFY RESET OTP — Step 2: verify OTP, issue reset cookie ────
...
export const verify2fa = catch_async(async (req, res) => {
  ...
  // Setup flow — first verification activates MFA
  if (!user.isMfaActive) {
